...
\begin{algorithmic}
\STATE{\textbf{Input:} A set $V$ consisting of $n$ points and a cost function $c$ : $V x V \rightarrow \mathbb{R}^+$ where the costs satisfy the triangle inequality.}
\STATE{\textbf{Output:} An $O$($\frac{logn}{log log n}$)-approximation to the ATSP instance described by the input's $V$ and $c$.}
\STATE{1. Solve the Held-Karp}
\end{algorithmic}
\end{algorithm}
\newline
